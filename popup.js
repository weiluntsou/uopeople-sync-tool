...
function buildCourseSummaryContent(course, unitMap, unitDetails, linkSuffix) {
    ...
    for (const [unit, data] of Object.entries(unitMap)) {
        ...
        if (isExamUnit) {
            ...
            chatPrompt += `───────────────────────────────────────\n\n`;
            chatPrompt += `  📋 Step 0. 章節性質窮舉清單 (Exhaustive Chapter Properties List)\n`;
            chatPrompt += `  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
            chatPrompt += `  ⚠️ 此步驟為最高優先級，必須在所有其他模組（心智模型圖、爭議地圖、作業藍圖等）之前完成輸出。\n\n`;
            chatPrompt += `  【無教材警語 — 條件觸發】\n`;
            chatPrompt += `  若使用者未提供任何教材（章節／投影片／指定閱讀），僅有作業題目，\n`;
            chatPrompt += `  請在清單最上方顯示：\n`;
            chatPrompt += `  「⚠️ 警告：本清單係依作業題目反推，未以完整教材為基礎，覆蓋率可能不全。\n`;
            chatPrompt += `    建議補充本週章節或投影片以確保完整性。」\n\n`;
            chatPrompt += `  執行規則：\n`;
            chatPrompt += `  1. 僅依據使用者提供的教材內容（章節／投影片／指定閱讀）。\n`;
            chatPrompt += `     禁止以 AI 自身知識庫補充任何未出現於教材的內容。\n`;
            chatPrompt += `  2. 逐一掃描教材中每一個：\n`;
            chatPrompt += `       定義 (definition)、公式 (formula)、性質陳述 (property statement)、\n`;
            chatPrompt += `       判斷準則 (decision criterion)、條件限制 (constraint)、常見對比 (common contrast)。\n`;
            chatPrompt += `     ► 不論其「重要性」或「深度」，只要符合本週 Topics 與 Learning Outcomes，\n`;
            chatPrompt += `       一律列為獨立條目。禁止篩選或省略任何簡單知識點。\n`;
            chatPrompt += `  3. 每一條目固定格式：\n`;
            chatPrompt += `       Q: [問句]\n`;
            chatPrompt += `       A: [答案，含完整公式（如適用）]\n`;
            chatPrompt += `  4. 條目數量：下限 15 條，上限 40 條。\n`;
            chatPrompt += `     ► 若不足 15 條，必須回頭重新掃描教材，\n`;
            chatPrompt += `       確認是否遺漏章節、摘要框 (summary box)、或關鍵詞表 (glossary)。\n`;
            chatPrompt += `  5. 若使用者同時提供測驗題目 (quiz/exam questions)：\n`;
            chatPrompt += `     ► 將每題的核心考點與上述清單逐一比對。\n`;
            chatPrompt += `     ► 未被清單覆蓋的考點，必須立即補入清單末尾，\n`;
            chatPrompt += `       並標註「⬅ 依據提供的測驗題目補充」。\n\n`;
            chatPrompt += `  【自我審核結果 — 輸出完畢後必須附上】\n`;
            chatPrompt += `     ✅ 條目數是否達到下限 15 條？（實際數量：___條）\n`;
            chatPrompt += `     ✅ 每條是否只含一個知識點？\n`;
            chatPrompt += `     ✅ 公式是否完整（無省略）？\n`;
            chatPrompt += `     ✅ 若有提供測驗題，是否每題考點均已覆蓋？\n\n`;
